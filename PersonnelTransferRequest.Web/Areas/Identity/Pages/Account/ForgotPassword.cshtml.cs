    ...
    public class ForgotPasswordModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            if (ModelState.IsValid)
            {
                ...
                var mailRquest = new MailRequest
                {
                    Body = $"Şifrenizi sıfırlamak için <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>linki</a> tıklayınız.",
                    Subject = "Şifre Sıfırlama",
                    ToEmail = Input.Email
                };

                await _mailService.SendEmailAsync(mailRquest);
